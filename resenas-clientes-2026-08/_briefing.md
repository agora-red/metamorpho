...
- **Interacción**: selector de modo que hace reaccionar los 18 touchpoints; estrellas clickeables (5★ vs ≤3★); switches de "qué se muestra" independientes; deep-link `?mode=` y `?stars=`.
- **v2 (25/8, pedido de Beto)**: mapear ABSOLUTAMENTE todos los lugares donde aparece el sistema — 18 touchpoints en 4 actos (WhatsApp, email con estrellas que puntúan, campanita del cliente, tabla Actividad, página de rating + variante 1–3★, gracias+Google, campanita del negocio "Tu bandeja", side menu en Marketing y comunicación, inbox, email al negocio, dashboard, ficha del cliente "Última actividad", equipo, storefront, marketplace, Google Maps, cierre del loop). Mocks fieles a las superficies reales relevadas en Flash/nightwing/cyclone (labels y layouts literales del código).
